...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const howMuchVectorsPointTheSameWay = x1 * x2 + y1 * y2;
  const angle1 = Math.sqrt(x1 * x1 + y1 * y1);
  const angle2 = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(howMuchVectorsPointTheSameWay / (angle1 * angle2));
}
...
function isPrime(n) {
  ...
  for (let i = 2; i <= Math.ceil(root); i += 1) {
    if (n % i === 0) {
      return false;
    }
  }
...
function getFibonacciNumber(index) {
  ...
  const conjugateOfGoldenRatio = (1 - Math.sqrt(5)) / 2;
  const result =
    (goldenRatio ** index - conjugateOfGoldenRatio ** index) / Math.sqrt(5);
